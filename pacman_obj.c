...
static bool pacman_movable(Pacman* pacman, Map* M, Directions targetDirec) {
	...
	if (is_wall_block(M, x, y, 0) || is_room_block(M, x, y))
		return false;
	if (is_wall_block(M, x, y, 1) || is_room_block(M, x, y))
		return false;
	
	return true;
}
void pacman_shoes(Pacman* pman) {
	if (al_get_timer_count(pman_speed_up) < 5) {
		pman->speed = basic_speed + 2;
	}
	else {
		pman->speed = basic_speed;
		al_stop_timer(pman_speed_up);
		al_set_timer_count(pman_speed_up, 0);
	}
}
	...
	if (game_over) {
		...
		*/
		for (int i = 0; i < 12; i++) {
			...
			if (al_get_timer_count(pman->death_anim_counter) / 7 == i) {
...
void pacman_eatItem(Pacman* pacman, const char Item) {
	...
	case '.':
		...
		break;
	case 'P':
		//game_abort("eat power bean\n");
		pacman_power_up(pacman);
		pacman->powerUp = true;
		break;
	case 'S':
		pacman_shoes(pacman);
		al_start_timer(pman_speed_up);
		break;
	case 'C':
		ghost_cage();
		break;
