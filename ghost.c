...
Ghost* ghost_create(int flag) {
	...
	case Blinky:
		ghost->objData.Coord.x = cage_grid_x-0.75;
	...
	case Clyde:
		...
		ghost->move_script = &ghost_yellow_move_script;
		break;
	default:
...
void ghost_draw(Ghost* ghost) {
	...
	if (ghost->status != FLEE && ghost->status != GO_IN) {
		switch (ghost->objData.facing) {
		default:
			if (ghost->objData.moveCD / 32 == 1) {
...
bool ghost_movable(Ghost* ghost, Map* M, Directions targetDirec, bool room) {
	...
	if (is_wall_block(M, x, y, 0) || (room && is_room_block(M, x, y)))
		return false;
	if (is_wall_block(M, x, y, 1) || (room && is_room_block(M, x, y)))
		return false;
...
void ghost_toggle_FLEE(Ghost* ghost, bool setFLEE) {
	...
	if(setFLEE){
		...
		if (ghost->status == FLEE) {
			...
			ghost->speed = basic_speed;
			if(ghost->typeFlag == 3) ghost->speed = basic_speed + 2;
		}
	}
}


void go_back_to_cage(Ghost* ghost) {
	game_log("ghost GO_IN");
	ghost->status = GO_IN;
	ghost->speed = 4;
}
...
void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	...
	for (Directions i = 1; i <= 4; i++) {
		if (i == banned || i == shortestDirection) continue;
		if (ghost_movable(ghost, M, i, 1)) 	proba[cnt++] = i;
	}
	if (cnt == 0) {
		if (ghost_movable(ghost, M, banned, 1) == 1) ghost_NextMove(ghost, banned);
