...
static void ghost_yellow_move_script_FREEDOM(Ghost* ghost, Map* M);
static void ghost_yellow_move_script_BLOCKED(Ghost* ghost, Map* M);

static void ghost_yellow_move_script_FREEDOM(Ghost* ghost, Map* M) {
	// [HACKATHON 2-4]
	// Uncomment the following code and finish pacman picking random direction.

	static Directions banned = 1;
...
void ghost_yellow_move_script(Ghost* ghost, Map* M, Pacman* pacman) {
	if (!movetime(ghost->speed))
		return;
		switch (ghost->status)
		{
		case BLOCKED:
			ghost_yellow_move_script_BLOCKED(ghost, M);
			if (al_get_timer_count(game_tick_timer) > GO_OUT_TIME)
				ghost->status = GO_OUT;
			break;
		case FREEDOM:
			ghost_yellow_move_script_FREEDOM(ghost, M);
			break;
		case GO_OUT:
