...
static void draw_block_index(Map* M, int row, int col, int type);
static void draw_bean(Map* M, const int row, const int col);
static void draw_power_bean(Map* M, const int row, const int col);
static void draw_shoes(Map* M, const int row, const int col);
static void draw_cage(Map* M, const int row, const int col);

extern ALLEGRO_BITMAP* shoes;
extern ALLEGRO_BITMAP* cage;
...
/* ----------------------------- start of default map code*/
	...
	*/
	...
	game_log("beans numeber = %d", M->beansCount);
	return M;
}
		...
		for (int col = 0; col < M->col_num; col++) {
			switch (M->map[row][col])
			{
				...
				case 'P':
					...
					break;
				case 'N':
					draw_block_index(M, row, col, 1);
					break;
				case '.':
					draw_bean(M, row, col);
					break;
				case 'S':
					draw_shoes(M, row, col);
					break;
				case 'C':
					draw_cage(M, row, col);
					break;
...
static void draw_block_index(Map* M, const int row, const int col, int type) {
	bool U = is_wall_block(M, col, row - 1, type);
	bool UR = is_wall_block(M, col + 1, row - 1, type);
	bool UL = is_wall_block(M, col -1, row- 1, type);
	bool B = is_wall_block(M, col, row+ 1, type);
	bool BR = is_wall_block(M, col + 1, row + 1, type);
	bool BL = is_wall_block(M, col - 1, row + 1, type);
	bool R = is_wall_block(M, col + 1, row, type);
	bool L = is_wall_block(M, col - 1,row, type);
	...
	dw = block_width / 3;
		...
		if(type == 0)al_draw_filled_rectangle(s_x, s_y,
					e_x, e_y, al_map_rgb(176, 196, 222));
		else if(type == 1)al_draw_filled_rectangle(s_x, s_y,
					e_x, e_y, al_map_rgb(174, 124, 233));
...
static void draw_shoes(Map* M, const int row, const int col) {
	al_draw_scaled_bitmap(shoes, 0, 0, 96, 96,
		map_offset_x + col * block_width + block_width / 6.0 - 5, map_offset_y + row * block_height + block_height / 4 - 8
		, 24, 24, 0);
}

static void draw_cage(Map* M, const int row, const int col) {
	al_draw_scaled_bitmap(cage, 0, 0, 96, 96,
		map_offset_x + col * block_width + block_width / 6.0 - 5, map_offset_y + row * block_height + block_height / 4 - 8
		, 24, 24, 0);
}

static void draw_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width/6.0,  al_map_rgb(64, 138, 212));
}
...
bool is_wall_block(Map* M, int index_x, int index_y, int type) {
	if (index_x < 0 || index_x >= M->col_num || index_y < 0 || index_y >= M->row_num)
		return true;
	if(type == 0) return M->map[index_y][index_x] == '#';
	else if(type == 1)return M->map[index_y][index_x] == 'N';
}
...
static	uint16_t end;
	...
	for (size_t i = 0; i < 4; i++) {
		...
		if (is_wall_block(M, x, y, 0) || steped[y][x])
			continue;
		if (is_wall_block(M, x, y, 1) || steped[y][x])
			continue;
		queue_x[end] = x;
		queue_y[end] = y;
		switch (i) {
